function board_list(page, type, mid,sort_index, search_word, tab){
	if(window.history.pushState){window.history.pushState(null,null,'/community/v3/main/'+tab);}
		var search = "";
		var search_option = "";
		...
		$.ajax({
            url: "/community/v3/main/board_list/"+page,
            type: "POST",
            data: {
            	type : type
            	, mid : mid
	...
	function board_view(document_srl, type, page, mid, sort_index, search, teacher_id,tab){
		if(window.history.pushState){window.history.pushState(null,null,'/community/v3/main/view_srl?document_srl='+document_srl+'&teacher_id='+teacher_id+'&tab='+tab);}
			$.ajax({
	            url: "/community/v3/main/board_view",
	            type: "POST",
	            data: {
	...
	function board_write(type, mid, sort_index, search, page, document_srl, teacher_id){
		$.ajax({
            url: "/community/v3/main/board_write",
            type: "POST",
            data: {
            	type : type
            	, mid : mid
	...
	function chang_lectures_list(teacher_id, mid, saleinfo_id){
		if(mid == 'china_teacher_review'){
			$.ajax({
	            url: "/community/v3/main/api_teacher_lectures",
	            type: "POST",
	            data: {
	            	teacher_id : teacher_id
	            },
	...
	function board_update(type, mid, page, search, sort_index, teacher_id, is_admin){
		if(version_check()){
			...
			if(confirm(text)){
				$.ajax({
		            url: "/community/v3/main/board_update_ajax",
		            type: "POST",
		            data: formData,
		            async: false,
		            cache: false,
	...
	function board_delete(document_srl, mid){
		if(confirm("정말 삭제하시겠습니까?")){
			$.ajax({
	            url: "/community/v3/main/board_delete",
	            type: "POST",
	            data: {
	            	document_srl : document_srl
	            },
	...
	function img_delete(id, file_srl){
		if(id == 'file_1_div'){
			$("#"+id).html("<input type=\"file\" id=\"file1\" name=\"files[]\">");
	        $("#file1_check").val('1');
	        $("#file1_srl").val(file_srl);
		}else{
			$("#"+id).html("<input type=\"file\" id=\"file2\" name=\"files[]\">");
	        $("#file2_check").val('1');
